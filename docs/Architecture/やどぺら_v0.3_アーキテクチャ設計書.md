...
**作成日**: 2025年11月21日  
**最終更新日**: 2026年3月21日  
**バージョン**: v0.3.7 (v0.2からのバージョンアップ) ※変更履歴に 2026-03-21 追記あり  
...
**v0.3実装詳細化（開発開始前明確化）**
13. **セッション統合トークン実装詳細**: 4桁英数字ランダム生成、重複チェック、試行回数制限、セキュリティ対策
14. **FAQ自動学習機能実装詳細**: GPT-4o miniプロンプト設計、カテゴリ自動推定、エラーハンドリング
15. **夜間対応キューMVP実装**: 手動実行ボタン、外部cron対応、タイムゾーン処理統一
16. **question_patterns更新ロジック**: 質問受付時のパターン記録、解決率計算、類似パターンマッチング
17. **信頼度スコア過去解決率取得**: pgvector検索による類似パターン検索、コスト最適化
18. **タイムゾーン処理統一**: 施設タイムゾーン基準での時刻判定、UTC保存・ローカル表示
19. **ゲストフィードバック低評価自動ハイライト**: 同一message_idへの低評価2回以上、管理画面表示
20. **OpenAI API障害時フォールバック詳細化**: 多言語対応、自動再試行なし、宿側ログ形式
21. **データ保持ポリシーMVP実装**: 手動削除機能、週次アラート通知、統計データ抽出
22. **Phase 0: 簡易ランディングページ（LP）作成**: 説明コスト削減、信頼性担保、PoC応募導線
23. **Phase 0: 開発環境構築詳細化**: Docker環境セットアップ、Backend/Frontend初期設定、README作成
24. **ブランチ戦略とデプロイ戦略**: オプション2採用（サブドメインでのテストURL作成）、ステージング環境はRailway Hobby使用
25. **実装フェーズ明確化**: ステージング環境構築はPhase 1 Week 4、本番環境構築はPhase 4

**v0.3.3追加（2025-12-20）**
26. **Phase 2機能追加**: 開発者管理ページ、デモサイト、宿泊施設管理者向けFAQ
27. **Phase 3機能追加**: 本番向けランディングページ作成
28. **Phase 4機能追加**: 新規登録ページ作成（Stripe実装直前）
26. **Phase 2-4の機能追加**（v0.3.3）: 開発者管理ページ、デモサイト、宿泊施設管理者向けFAQ、本番向けランディングページ、新規登録ページ

**v0.3.4追加（2025-12-21）**
29. **料金プラン改訂**: Freeプラン追加（1言語、30質問限定、超過後はFAQ対応のみ、20件、〜25床）、同時利用言語数の概念追加、各プランの言語数・価格・FAQ登録数・規模目安を更新（Mini: 2言語、Small: 3言語、Standard: 4言語、Premium: 無制限言語）

**v0.3.7追加（2025-12-22）**
30. **Phase 3実装予定: QRコードデザイン変更**: QRコードの中央に「YadOPERA」テキストを配置、QRコードの下部に情報を追加（設置場所、生成日時、施設名など）。PNG/PDF/SVG形式すべてに対応。実装箇所: `backend/app/services/qr_code_service.py`の`generate_qr_code()`メソッド

**v0.3.8追記（2026-03-21・文書のみ）**
31. **静的 LP（`landing/`）と GitHub Pages**: カスタムドメイン `yadopera.com` の静的サイトは **GitHub Actions**（`main` の `landing/**`）で配信。**`develop` の `landing/` 変更だけでは本番URLは更新されない**。ティザーと本番LPで `main` / `develop` の `index.html` 役割が分岐しうる。詳細・教訓は `docs/20260321_LPティザー_GitHubPages_ブランチとデプロイ不手際_記録.md`。Render 上のフロントステージングとは別パイプライン。
...
16. [開発規約](#16-開発規約)
17. [開発計画・Phase 0（準備期間）](#17-開発計画phase-0準備期間)
17. [開発計画・Phase 0（準備期間）](#17-開発計画phase-0準備期間)
...
QRコード読み取り
    ↓
/f/:facility_slug?location=entrance&token={session_token}
    ↓
Cookie確認
    ├─ tabipera_session 存在
    │   ├─ セッション有効性チェック
...
#### セッション統合トークン生成実装（v0.3詳細化）

```python
# app/services/session_token_service.py
...
class SessionTokenService:
    ...
    async def link_session(
        ...
        """
        # トークン検索
        session_token = await db.execute(
            select(SessionToken)
            .where(SessionToken.token == token)
        )
        token_obj = session_token.scalar_one_or_none()
        ...
        # 試行回数制限チェック（Redis使用、Phase 2で実装）
        # MVP期間中は簡易実装（データベースカラム追加検討）
...
CREATE TABLE facilities (
    ...
    subscription_plan VARCHAR(50) DEFAULT 'small',  -- 'free', 'mini', 'small', 'standard', 'premium'
...
-- FAQ（インテント）テーブル: 意味（インテント）単位で1件として管理
CREATE TABLE faqs (
    id SERIAL PRIMARY KEY,
    facility_id INTEGER NOT NULL REFERENCES facilities(id) ON DELETE CASCADE,
    category faq_category NOT NULL,
    intent_key VARCHAR(100) NOT NULL,  -- v0.3.5追加: インテント識別キー（例: 'basic_checkout_time'）
    priority INTEGER DEFAULT 1,  -- 1-5
    is_active BOOLEAN DEFAULT TRUE,
    created_by INTEGER REFERENCES users(id) ON DELETE SET NULL,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(facility_id, intent_key)  -- v0.3.5追加: 同一施設内で同じインテントは1件のみ
);
...
CREATE INDEX idx_faqs_facility_intent ON faqs(facility_id, intent_key);
```

#### faq_translations (FAQ翻訳) - v0.3.5追加
...
CREATE TABLE faq_translations (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(faq_id, language)  -- 同じFAQ（インテント）の同じ言語は1件のみ
);
...
CREATE INDEX idx_faq_translations_embedding ON faq_translations 
USING ivfflat (embedding vector_cosine_ops)
WITH (lists = 100);  -- データ量に応じて調整

-- 埋め込みベクトルがNULLでないFAQ翻訳のみインデックス対象
-- （新規作成時はNULL、保存時に自動生成）
```

**v0.3.5変更点**:
- FAQ登録数は「意味（インテント）」単位で1件としてカウント
- 多言語対応してもFAQ登録数が増えない設計（例：「今何時ですか？」と「what time is it now?」は1件としてカウント）
- `faqs`テーブル: `language`, `question`, `answer`, `embedding`を削除、`intent_key`を追加
- `faq_translations`テーブル: 新規作成（言語ごとの質問・回答・埋め込みベクトルを保存）
...
**question_patternsテーブルの更新ロジック（v0.3詳細化）**

```python
# app/services/question_pattern_service.py
...
class QuestionPatternService:
    ...
    SIMILARITY_THRESHOLD = 0.85  # 同一パターンと判定するコサイン類似度閾値
    
    async def update_pattern(
        self,
        facility_id: int,
        question: str,
        is_resolved: bool,  # エスカレーションなしで完了したか
        db: AsyncSession
    ) -> QuestionPattern:
        """
        質問パターンの更新（v0.3新規）
        - 質問受付時に埋め込みベクトル生成
        - 類似パターン検索（コサイン類似度0.85以上）
        - 存在すればtotal_count+1、解決済みならresolved_count+1
        - 存在しなければ新規作成
        """
        # 質問の埋め込みベクトル生成
        question_embedding = await generate_embedding(question)
        embedding_vector = f"[{','.join(map(str, question_embedding))}]"
        ...
        if row and row.similarity >= self.SIMILARITY_THRESHOLD:
            ...
            pattern = QuestionPattern(
                facility_id=facility_id,
                pattern_embedding=question_embedding,
                total_count=1,
    ...
    async def search_similar_patterns(
        ...
        """
        類似質問パターン検索（v0.3新規）
        pgvectorでコサイン類似度0.85以上を同一パターンと判定
        """
        ...
        query = select(
            ...
            (1 - func.cosine_distance(QuestionPattern.pattern_embedding, embedding_vector)) >= threshold
        ).order_by(
            func.cosine_distance(QuestionPattern.pattern_embedding, embedding_vector).asc()
        ).limit(top_k)
        
        result = await db.execute(query)
        patterns = [row.QuestionPattern for row in result.all()]
...
**MVP期間中（Phase 1）実装詳細（v0.3追加）**

**1. 手動削除機能（管理画面）**
- 管理画面に「データ削除」セクションを追加
- 3ヶ月経過データの一覧表示
- 施設別・日付範囲別フィルタリング
- 一括削除機能（確認ダイアログ付き）

```python
# app/api/v1/admin/data_cleanup.py

@router.get("/admin/data-cleanup/list")
async def list_expired_data(
    facility_id: int,
    ...
    # 削除対象データを取得
    expired_conversations = await db.execute(
        select(Conversation)
        .where(
            Conversation.facility_id == facility_id,
            Conversation.ended_at < three_months_ago
        )
    )
    
    expired_escalations = await db.execute(
        select(Escalation)
        .where(
            Escalation.facility_id == facility_id,
            Escalation.created_at < three_months_ago,
            Escalation.resolved_at.isnot(None)
        )
    )
    
    return {
        "expired_conversations": len(expired_conversations.scalars().all()),
        "expired_escalations": len(expired_escalations.scalars().all()),
        "total_size_mb": 0  # Phase 2で実装
    }

@router.post("/admin/data-cleanup/delete")
...
**2. アラート通知（週次メール）**
- 毎週月曜日に3ヶ月経過データのアラートメール送信
- 削除推奨データ件数、データサイズを通知
- 管理画面へのリンクを含む

```python
# app/utils/data_cleanup_notification.py

async def send_cleanup_alert(
    facility_id: int,
    db: AsyncSession
) -> None:
    """
    ...
    count = expired_count.scalar()
    
    if count > 0:
        facility = await db.get(Facility, facility_id)
        subject = f"[やどぺら] データ削除のご案内: {facility.name}"
        
        body = f"""
施設名: {facility.name}

3ヶ月経過した会話データが {count} 件あります。
GDPR準拠のため、削除をお勧めします。

管理画面で削除できます:
https://admin.yadopera.com/admin/data-cleanup

削除は手動で実行してください。
"""
        
        recipients = await get_facility_admin_emails(facility_id)
...
| **GET** | **`/admin/low-rated-messages`** | **低評価回答一覧** | **JWT** | **★新規** |
| **GET** | **`/admin/data-cleanup/list`** | **3ヶ月経過データ一覧** | **JWT** | **★新規** |
| **POST** | **`/admin/data-cleanup/delete`** | **データ削除実行** | **JWT** | **★新規** |
| **POST** | **`/admin/overnight-queue/process`** | **夜間対応キュー処理（手動実行）** | **JWT** | **★新規** |
...
### 10.7 セッション統合トークンのセキュリティ対策（v0.3新規）

#### トークン推測攻撃対策

**1. 試行回数制限**
- 同一IPアドレスから1時間あたり最大5回のトークン入力試行を許可
- 5回超過時は1時間ブロック
- MVP期間中: Redisまたはデータベースで実装
- Phase 2: Redis Rate Limitingを使用
...
class SessionTokenService:
    ...
    async def check_rate_limit(
        ...
        """
        トークン入力試行回数制限チェック（v0.3新規）
        """
        # Redisキー: session_token_attempts:{ip_address}
        key = f"session_token_attempts:{ip_address}"
        ...
        await redis_client.expire(key, 3600)  # 1時間有効
    ...
    async def block_ip(
        ...
        IPアドレスをブロック（v0.3新規）
        """
        key = f"session_token_blocked:{ip_address}"
        await redis_client.setex(
            key,
            self.BLOCK_DURATION_HOURS * 3600,
            "1"
        )
```
...
- トークンは24時間固定（延長機能なし）
- 期限切れトークンは即座に無効化
- 期限切れトークンでの統合試行はエラーを返す

**3. トークン生成のランダム性**
- `random.choices()`を使用したランダム生成
- 予測不可能性を確保
- トークン空間: 36^4 = 1,679,616通り

**4. トークン重複チェック**
- データベースのUNIQUE制約で重複防止
- 重複時は自動再生成（最大10回）
- 10回失敗時はエラーを返す（極めて稀）
...
async def verify_token(
    ...
    """
    # 試行回数制限チェック
    if not await self.check_rate_limit(ip_address, db):
        raise HTTPException(
            status_code=429,
...
async def calculate_confidence(
    ...
    similar_patterns = await search_similar_patterns(
        facility_id=facility_id,
        embedding=question_embedding,
        threshold=0.85,  # コサイン類似度0.85以上を同一パターンと判定
    ...
    if similar_patterns:
        ...
        if pattern.resolution_rate >= 0.8:
            base_confidence += 0.15  # +0.15ボーナス
...
### 11.4 FAQ自動学習機能（v0.3新規）

#### FAQ自動学習フロー実装

```python
# app/services/faq_suggestion_service.py
...
class FAQSuggestionService:
    ...
    async def create_suggestion_from_message(
        self,
        facility_id: int,
        message_id: int,
        question: str,
        ...
        """
        未解決質問からFAQ提案を作成（v0.3新規）
        - 質問文を自動入力
        - 回答文テンプレートをGPT-4o miniで自動生成
        - カテゴリを自動推定
        """
        # 回答文テンプレート自動生成
        answer_template = await self._generate_answer_template(
            question=question,
            language=language
        )
        
        # カテゴリ自動推定
        category = await self._estimate_category(question)
    ...
    async def _generate_answer_template(
        self,
        question: str,
        language: str = 'en'
    ) -> str:
        """
        回答文テンプレート自動生成（GPT-4o mini使用、v0.3新規）
        """
        prompt = f"""You are a helpful assistant for a guesthouse.

Based on the following guest question, generate a concise and helpful answer template (under 200 characters).
...
3. Is concise (under 200 characters)
4. Can be customized by staff later
...
Answer template:"""
        ...
        try:
            response = await self.openai_client.chat.completions.create(
                model="gpt-4o-mini-2024-07-18",
                messages=[
                    {"role": "system", "content": "You are a helpful assistant for a guesthouse."},
            ...
            answer_template = response.choices[0].message.content.strip()
            return answer_template
        
        except Exception as e:
            # 生成失敗時は空文字列を返す（スタッフが手入力可能）
            # エラーログを記録
            print(f"Failed to generate answer template: {e}")
            return ""
    ...
    async def _estimate_category(
        ...
        """
        カテゴリ自動推定（GPT-4o mini使用、v0.3新規）
        """
        prompt = f"""Classify this guest question into ONE of the following categories:
- basic: Check-in/out, WiFi password, payment, curfew
- facilities: Kitchen, shower, laundry, towels, dryer, luggage storage, bicycle rental
- location: Convenience store, station, restaurant, ATM, tourist access
- trouble: Lost key, WiFi issues, equipment failure

Question: {question}

Respond with ONLY the category name (basic, facilities, location, or trouble)."""
        
        try:
            response = await self.openai_client.chat.completions.create(
                model="gpt-4o-mini-2024-07-18",
                messages=[
                    {"role": "system", "content": "You are a category classifier for guesthouse questions."},
                    {"role": "user", "content": prompt}
                ],
                max_tokens=10,
                temperature=0
            ...
            # 推定失敗時はデフォルト 'basic' を返す
            print(f"Failed to estimate category: {e}")
            return 'basic'
    ...
    async def approve_suggestion(
        ...
        # 編集された値を使用（なければ元の値）
        question = edited_question or suggestion.suggested_question
        answer = edited_answer or suggestion.suggested_answer
        category = edited_category or suggestion.suggested_category
        
        # FAQ作成
        faq = FAQ(
            facility_id=suggestion.facility_id,
            category=category,
            language='en',  # MVPは英語のみ
            question=question,
            answer=answer,
            created_by=user_id,
            priority=1,
        ...
        # 埋め込みベクトル自動生成
        faq.embedding = await generate_embedding(f"{question} {answer}")
...
class FeedbackService:
    """
    ゲストフィードバック管理サービス（v0.3新規）
    """
    
    async def get_low_rated_messages(
        self,
        facility_id: int,
        threshold: int = 2
    ) -> List[dict]:
        ...
        - 管理画面の「未解決質問リスト」に「低評価」バッジ表示
        """
        # 低評価が2回以上のメッセージを取得
        query = (
            select(
                Message,
                func.count(GuestFeedback.id).label('negative_count')
            )
            .join(GuestFeedback, Message.id == GuestFeedback.message_id)
            .where(
                GuestFeedback.facility_id == facility_id,
                GuestFeedback.feedback_type == 'negative'
            )
            .group_by(Message.id)
            .having(func.count(GuestFeedback.id) >= threshold)
        )
        ...
        for row in rows:
            messages.append({
                ...
                "highlight": True  # ハイライトフラグ
            })
        
        return messages
    
    async def generate_faq_improvement_suggestion(
        self,
        message_id: int,
        language: str = 'en'
    ) -> str:
...
The following AI answer received negative feedback from a guest. Please suggest an improved version.

Original answer: {message.content}

Generate an improved answer that:
1. Is more helpful and accurate
2. Directly addresses the guest's concern
3. Is concise (under 200 characters)
...
Improved answer:"""
        ...
        try:
            ...
            print(f"Failed to generate improvement suggestion: {e}")
            return ""
```

### 11.5 FAQ埋め込みベクトル自動生成
...
信頼度スコア取得
    ↓
現在時刻・曜日・言語取得（施設タイムゾーン基準）★v0.3改善
    ├─ 施設のtimezoneカラム取得（デフォルト: 'Asia/Tokyo'）
    ├─ UTC → 施設タイムゾーンに変換
    └─ ローカル時刻で判定
    ↓
エスカレーションスケジュール検索
...
class OvernightQueueService:
    ...
    async def process_scheduled_notifications(
        ...
        """
        翌朝8:00の一括通知処理（v0.3詳細化）
        
        MVP期間中（Phase 1）:
        - 管理画面から手動実行ボタン
        - または外部cronから定期API呼び出し: POST /api/v1/admin/overnight-queue/process
        
        Phase 2:
        - Celeryバッチ処理で自動実行（毎日8:00、施設タイムゾーン別）
        """
        now_utc = datetime.utcnow()
        
        # 各施設のタイムゾーンに応じて8:00-8:30の範囲で未通知のキューを取得
        # （簡易実装: UTC 23:00-23:30を対象、施設タイムゾーンは'Asia/Tokyo'想定）
        # Phase 2で施設タイムゾーン別に処理するよう改善
        queues = await db.execute(
            select(OvernightQueue)
            .join(Facility, OvernightQueue.facility_id == Facility.id)
            .where(
                OvernightQueue.scheduled_notify_at <= now_utc,
                OvernightQueue.scheduled_notify_at >= now_utc - timedelta(minutes=30),
...
**動作（v0.3改善）**
- 施設のタイムゾーン基準で判定（facilities.timezone、デフォルト: 'Asia/Tokyo'）
- 平日9時-18時の間（施設のローカル時刻）
- 信頼度 < 0.85でエスカレーション
- 英語・日本語対応
- メール通知
...
**動作（v0.3改善）**
- 施設のタイムゾーン基準で判定
- 全曜日18時-翌9時の間（施設のローカル時刻）
- 信頼度 < 0.7でエスカレーション
- 緊急キーワード検出時は即座に通知
- 夜間時間帯（22:00-8:00）は夜間対応キューに追加★v0.3新規
...
### 13.3 OpenAI API障害時フォールバック（v0.3詳細化）

#### 多言語対応フォールバックメッセージ（v0.3新規）

```python
# app/ai/fallback_messages.py
...
#### OpenAI API障害時の詳細処理（v0.3詳細化）

```python
# app/ai/openai_client.py

from openai import OpenAI, OpenAIError
from app.ai.fallback_messages import get_fallback_message
import logging
...
class OpenAIClient:
    ...
    async def generate_response(
        ...
        try:
            ...
            logger.error(
                "OpenAI API error: fallback_message_triggered",
                extra={
                    "timestamp": datetime.utcnow().isoformat(),
                    "error_type": error_type,  # OpenAI_API_timeout / rate_limit / server_error
            ...
            # 宿側ログ記録（データベース）
            await self._log_api_error(
                error_type=error_type,
                error_message=error_message,
                language=language
            )
            
            # フォールバックメッセージ返却（ゲストの選択言語に応じて）
    ...
    async def _log_api_error(
        ...
        宿側管理画面で確認可能
        """
        # エラーログテーブルに記録（Phase 2で実装）
        # MVP期間中はアプリケーションログのみ
        pass
```

#### エラーログ形式（宿側管理画面表示用、v0.3新規）
...
```json
{
  ...
  "facility_id": 1,
  "language": "en",
  "error_message": "Request timeout after 5.0 seconds"
}
```

**管理画面での表示:**
- エラーログ一覧画面（Phase 2で実装）
- エラー発生時のアラート通知（Phase 2で実装）
- MVP期間中はアプリケーションログで確認

#### 自動再試行ポリシー（v0.3明確化）

**方針: 自動再試行なし**
- 理由: ゲスト体験の一貫性、無限ループ防止
- 代替: ゲストに再送信を促すUI表示
- 実装: フロントエンドで「再送信」ボタン表示
...
<template>
  <div v-if="error" class="error-message">
    <p>{{ errorMessage }}</p>
    <button @click="retryMessage">再送信</button>
  </div>
</template>
```
...
class OpenAIClient:
    ...
    async def generate_response(
        ...
        try:
            ...
            # ゲストの選択言語に応じたフォールバックメッセージ返却（v0.3改善）
            return self.FALLBACK_MESSAGES.get(language, self.FALLBACK_MESSAGES["en"])
...
**ブランチ戦略（オプション2: サブドメインでのテストURL作成）**
...
#### ステージング環境（Render.com + Railway Hobby）
...
│  │  - Health Check: /api/v1/health│ │
│  │  - URL: https://staging-api.yadopera.com│ │
│  └───────────────────────────────┘ │
│                                     │
│  ┌───────────────────────────────┐ │
│  │  Static Site (Vue.js)         │ │
...
#### Backend - ステージング環境 (.env.staging)

```bash
# Database (Railway Hobby PostgreSQL)
DATABASE_URL=postgresql+asyncpg://user:pass@railway-host:5432/yadopera_staging
...
SMTP_USER=noreply@yadopera.com
SMTP_PASSWORD=xxxxx
FRONTEND_URL=https://staging.yadopera.com

# CORS
FRONTEND_URL=https://staging.yadopera.com
ADMIN_URL=https://staging-admin.yadopera.com

# AWS S3 (QRコード保存、ステージング用バケット)
AWS_ACCESS_KEY_ID=xxxxx
AWS_SECRET_ACCESS_KEY=xxxxx
AWS_S3_BUCKET=yadopera-qrcodes-staging
AWS_REGION=ap-northeast-1

# Encryption
ENCRYPTION_KEY=xxxxx  # Fernet key (ステージング用)

# Environment
ENVIRONMENT=staging
DEBUG=true  # ステージングではデバッグ有効
```

#### Frontend - 本番環境 (.env.production)
...
#### Frontend - ステージング環境 (.env.staging)

```bash
VITE_API_BASE_URL=https://staging-api.yadopera.com
...
#### Renderサーバー環境設定とデータベース設定の実装タイミング

**Phase 1 Week 4: 統合・テスト**
- [ ] ステージング環境構築
  - [ ] `develop`ブランチ作成
...
**実装順序**:
1. **Phase 1 Week 4**: ステージング環境構築（開発・テスト用）
...
#### ステージング環境設定（Render.com + Railway Hobby）

**Web Service設定（ステージング）**

```yaml
# render.yaml (ステージング環境)

services:
  - type: web
    name: yadopera-api-staging
    env: python
    ...
    envVars:
      - key: DATABASE_URL
        sync: false  # Railway Hobby PostgreSQL（外部）
        value: ${RAILWAY_DATABASE_URL}  # Railwayから取得
      - key: REDIS_URL
        sync: false  # Railway Hobby Redis（外部）
        value: ${RAILWAY_REDIS_URL}  # Railwayから取得
      - key: OPENAI_API_KEY
        sync: false
      - key: SECRET_KEY
        generateValue: true
...
```
Railway Hobby プロジェクト:
- PostgreSQL 15
  - Plan: Hobby
  - Storage: 1GB
  - Extensions: pgvector（手動有効化）
  - Connection URL: ${RAILWAY_DATABASE_URL}

- Redis
  - Plan: Hobby
  - Connection URL: ${RAILWAY_REDIS_URL}
```

**注意事項**:
- Railway HobbyのPostgreSQLにpgvector拡張を有効化する必要がある
- Railwayの環境変数をRender.comの環境変数に設定する必要がある
...
### 16.9 ご利用マニュアル（管理画面）の更新・改訂

管理画面の「ご利用マニュアル」を更新・改訂する際は、**必ず次の必読文書に従うこと**。コードを変更しただけではブラウザに反映されず、PWA の Service Worker やキャッシュにより古い表示のままになる事象を防ぐためである。

- **必読**: `docs/マニュアル更新・改訂_必読.md`
- **反映手順の詳細**: `docs/マニュアルManual編集後の反映手順.md`
- **対象ファイル**: `frontend/src/views/admin/Manual.vue`（本文）、`frontend/src/components/admin/ManualContent.vue`（表示・スタイル）
- **編集後の必須手順**: フロントの再ビルド・再起動 → ブラウザでサイトデータ消去・Service Worker 解除 → ハードリロード。詳細は必読文書を参照。

---

## 17. 開発計画・Phase 0（準備期間）

### 17.1 Phase 0: 準備（1週間）

Phase 0では、MVP開発開始前の準備タスクを実施します。

#### 必須タスク

1. **アーキテクチャ設計書v0.3作成** ✅完了
   - 本ドキュメントの作成・更新
   - v0.3新機能の実装詳細化

2. **簡易ランディングページ（LP）作成**★新規 ✅完了（2025-11-25）
   - **目的**:
     - 説明コスト削減（メールDM・説明会前の事前説明）
     - 信頼性担保（Webプレゼンスによる安心感）
     - PoC応募導線（応募フォーム・説明会予約の集約）
   - **実装状況**:
     - ✅ HTML実装完了（12セクション構成）
     - ✅ レスポンシブデザイン対応
     - ✅ PoC応募フォーム実装（プレースホルダー）
     - ✅ 説明会予約セクション実装
     - ✅ Google Analytics 4実装（測定ID未設定、後回し）
     - ⚠️ PoC説明不足（重要）: PoCとは何か、契約内容、関係性の説明が必要
     - ⏳ フォーム送信機能の実際のサービス接続（Google Forms、Formspree等）
     - ⏳ デプロイ（Vercel予定）
   - **構成**:
     | セクション | 内容 | 目的 |
     |-----------|------|------|
     | ヒーロー | キャッチコピー + 1分デモ動画 | 興味喚起 |
     | 課題提起 | 「こんなお悩みありませんか？」 | 共感獲得 |
     | 解決策 | やどぺらの3つの特徴 | 価値訴求 |
     | 仕組み | QR→AI応答のフロー図 | 理解促進 |
     | 料金 | PoC無料 + 本導入後プラン | 安心感 |
     | FAQ | よくある質問5-7項目 | 不安解消 |
     | CTA | PoC応募フォーム + 説明会予約 | コンバージョン |
   - **技術仕様**:
     - 静的HTML/CSS/JavaScript（Tailwind CSS CDN使用）
     - レスポンシブデザイン（スマホ最適化）
     - フォーム送信: 外部サービス（Google Forms、Typeform等）または簡易API（プレースホルダー実装）
     - Google Analytics 4: 実装済み（測定ID未設定、デプロイ前に対応）
     - デプロイ: Vercel予定（カスタムドメイン: yadopera.com）
   - **工数**: 実装完了（ブラウザテスト完了、修正項目あり）
   - **ブラウザテスト結果**（2025-11-25）:
     - ✅ ページ表示: 正常
     - ✅ セクション表示: 12セクションすべて表示
     - ✅ レスポンシブデザイン: 動作確認済み
     - ⚠️ コンソール警告: Tailwind CDN本番環境警告（開発環境のため問題なし）、favicon.ico 404（機能に影響なし）
     - ⏳ 未確認: フォーム動作、デザイン・カラーリング、文言・誤植
     - ⚠️ 問題発見: PoC説明不足（重要）
   - **デモ動画内容**:
     - QRコード読み取り
     - 多言語選択
...
3. **開発環境構築**（詳細化）
   - **プロジェクト構造作成**
     ...
     - `.gitignore`設定（Python, Node.js, Docker用）
   - **Docker環境セットアップ**
     - `docker-compose.yml`作成（PostgreSQL + pgvector, Redis, Backend, Frontend）
     - `backend/Dockerfile`作成
     - `frontend/Dockerfile`作成
     - `docker-compose up`で全サービス起動確認
   - **Backend初期設定**
     - `backend/requirements.txt`作成（FastAPI, SQLAlchemy, Alembic, pgvector, OpenAI等）
     - `backend/.env.example`作成（環境変数テンプレート）
     - `backend/alembic.ini`作成
     - Alembic初期化（`alembic init alembic`）
     - 初期マイグレーションファイル作成（pgvector拡張有効化）
     - FastAPIプロジェクト骨格作成（`app/main.py`, `app/core/config.py`等）
   - **Frontend初期設定**
     - `frontend/package.json`作成（Vue 3, TypeScript, Vite等）
     - `frontend/.env.example`作成（環境変数テンプレート）
     - Vite + Vue 3プロジェクト初期化
     - TypeScript設定（`tsconfig.json`）
     - ESLint, Prettier設定
   - **データベース初期化**
     - PostgreSQL + pgvector環境構築確認
     - pgvector拡張有効化確認
     - 接続テスト
   - **Redis環境構築**
     - Redis接続確認
   ...
   - **動作確認**
     ...
     - Backend: `http://localhost:8000/docs`でSwagger UI表示確認
     - Frontend: `http://localhost:5173`でVite開発サーバー起動確認
     - PostgreSQL: `psql`接続確認
     - Redis: `redis-cli`接続確認
...
### 17.3 Phase 2: PoC準備（2週間）

Phase 1完了後、Phase 2（PoC準備）を開始します。
...
- **デモサイト実装**★新規（v0.3.3）
  ...
  - 実際のシステム機能を体験できる環境
  - 管理画面とゲスト画面の両方を体験可能
- **宿泊施設管理者向けFAQ実装**★新規（v0.3.3）
  - 宿泊管理者が困惑しないよう本システムと同様のFAQを設置
  - 管理画面内にFAQページを追加
  - よくある質問と回答を提供
  - カテゴリ別に整理（初期設定、QRコード設置、FAQ登録、AI応答の仕組み、ログ・分析、トラブルシューティング、料金・請求、セキュリティ・プライバシー）

### 17.4 Phase 3: PoC実施（2ヶ月）

Phase 2完了後、Phase 3（PoC実施）を開始します。
...
- 成功事例の記録
- 有料転換率測定
- **本番向けランディングページ作成**★新規（v0.3.3）
  - SNSなどから誘引するページで申し込み受付できるページ
  - ヒーローセクション、機能紹介、料金プラン、申し込みフォーム
  - SEO最適化、SNSシェアボタン
  - 問い合わせフォーム、FAQセクション
  - デモ動画、お客様の声、導入事例
- **管理者ダッシュボードページに月次サマリー統計カードをメインで実装**★新規（v0.3.4）
  - 管理者は月次統計と料金が一番の関心課題なので、月次サマリー統計カードをメインで実装
  - のちにStripeとリンク
  - 請求期間ベースの統計カード（総質問数、AI自動対応数、自動応答率、エスカレーション数、想定人手対応時間削減）
  - 品質・安心系カード（平均AI信頼度、ゲスト満足度（👍率））
- **プラン超過時の挙動の実装**★新規（v0.3.4）→ **2026-03-13 実装済み**
  - 管理者選択制で、プラン超過時の挙動を実装。管理画面「プラン・請求」で選択・保存可能（Free/Small/Standard/Premium 時表示、Mini は非表示）
  - 通常継続（従量課金）/AI停止・FAQ限定モード。参照: `docs/プラン超過時の挙動_管理者選択制_実装計画.md`
- **QRコードデザイン変更**★新規（v0.3.7、2025-12-22追加）
...
### 17.5 Phase 4: 本格展開準備（1ヶ月）

Phase 3完了後、Phase 4（本格展開準備）を開始します。
...
- **新規登録ページ作成**★新規（v0.3.3）
  ...
  - 利用規約・プライバシーポリシーへの同意
  - メール認証機能
  - 施設情報登録（施設名、住所、電話番号、施設タイプ、ベッド数など）
- 決済機能実装（Stripe連携）。領収証のインボイス対応・請求書/領収書の日本語表示は `docs/領収証_インボイス対応化_設定計画.md` を参照。
...
以上で「やどぺら v0.3 アーキテクチャ設計書」の全17セクションと付録が完成しました。
