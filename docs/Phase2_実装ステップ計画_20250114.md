...
- ✅ 統合ヘルプシステム実装（宿泊事業者向けAIヘルプチャット） - **完了（2025-01-14）**
  ...
  - Frontend実装完了
  - 統合テスト・ブラウザテスト完了
  - ステージング環境デプロイ完了（2025-01-14）
  - ステージング環境初期データ投入完了（2025-01-14）
  - ⚠️ 残存課題: 
    - FAQ内容の修正が必要（誤った記述を数件発見）
    - FAQ関連URLの修正が必要（存在しないページや間違ったURLが含まれている）
- ✅ 月次ダッシュボード統計実装（施設管理者のコスト意識に対応） - **完了（2026-01-16）**
  - データベース拡張完了（マイグレーション012）
  - Backend API実装完了（`get_monthly_usage`, `get_ai_automation`, `get_escalations_summary`, `get_unresolved_escalations`）
  - Frontend実装完了（`MonthlyUsageCard`, `AiAutomationCard`, `EscalationsSummaryCard`, `UnresolvedListCard`）
  - エラーハンドリング統一化完了（2026-01-16）
  - 空文字列`session_id`チェック追加完了（2026-01-16）
  - ステージング環境デプロイ完了（2026-01-16）
  - コミット: `d6e46f7`（`develop`ブランチ）
- ✅ 利用マニュアル実装 - **完了（2026-01-17）**
  - 全12章、55項目の本文内容を実装完了
  - デプロイ完了、ブラウザテスト完了
  - コミット: `e87c832`（`develop`ブランチ）
- ⏳ ティザーサイト作成・公開
- ⏳ 開発者管理ページ実装
...
| 2. 月次ダッシュボード統計 | 高 | 2-3日 | ✅ 完了 | 施設管理者のコスト意識に対応、最重要、エラーハンドリング統一化完了（2026-01-16） |
| 3. 利用マニュアル | 中 | 3-4時間 | ✅ 完了 | 優先度9/9（最終項目）、全12章実装完了（2026-01-17） |
| 4. ティザーサイト作成・公開 | 中 | 2-3時間 | 未着手 | PoC進行中〜正式ローンチ直前（2026年2月〜4月初旬公開予定） |
| 5. 開発者管理ページ | 中 | 10-14時間 | 未着手 | 優先度7番目 |
| 6. PWA404エラー問題 | 低 | - | 残存課題 | 6セッション以上取り組んだが未解決 |
...
**所要時間**: 3-4時間（基本実装）+ 追加作業（本文内容実装）  
**参照文書**: `docs/manual_plan.md`  
**詳細引き継ぎ書**: `docs/20260117_142813_YadOPERA利用マニュアル全12章実装完了_セッション引き継ぎ書.md`

#### ステップ4: ティザーサイト作成・公開

**理由**:
- PoC進行中に関心を持った宿泊施設に「あとで正式に確認したい」という保留フラグを立ててもらう
- 本番LP公開前に、思想・方向性を正しく伝える
- 想定公開期間: 2026年2月〜4月初旬（PoC進行中〜正式ローンチ直前）

**実装内容**:
1. **ティザーサイトHTML作成**（1-1.5時間）
   - `landing/index.html`または新規`teaser/index.html`を作成
   - 構成要素:
     ...
     - PoC進行中の補足（任意・控えめ）
   - スタイリング: Tailwind CSS（CDN使用）
   - レスポンシブ対応（モバイル・タブレット・デスクトップ）

2. **フォーム送信設定**（30分）
   - Formspreeまたは同様のサービスを使用
   - メール登録の扱い: サービス開始連絡のみ（1通だけ送信、メルマガ・営業メールには使用しない）

3. **デプロイ設定**（30分）
   - GitHub Pagesまたは同様のホスティングサービスでデプロイ
   - ルーティング設定: `/` → ティザーHTML
   - `/landing/` → 本番LP（noindex、URL直打ち or 個別案内のみ）
...
5. **公開準備**（30分）
   - SEO設定（noindex、description）
   - アクセス解析設定（Google Analytics等、任意）

**重要な注意事項**:
- **出さないこと**: 機能一覧、価格詳細、導入効果の数値、補助金・制度説明、競合比較、デモ画面
- **使わない言葉**: 「AI」「多言語」「自動化」→ ティザーでは1文字も使わない
- **数値**: 一切出さない（%・円）
- **導入効果**: 本番LPで初めて語る

**公開スケジュール**:
- **フェーズ1（2026年2月〜3月）**: `/` → ティザーHTML、`/landing/` → noindex（URL直打ち or 個別案内のみ）
- **フェーズ2（2026年4月ローンチ直前）**: ティザー登録者に1通通知、`/`を本番LPに切替、ティザーHTMLは`/teaser-closed.html`等で保存

**所要時間**: 約2-3時間（0.5日）  
**参照文書**: `docs/teaser_summary.md`  
**実装ファイル**: 
- `landing/index.html`（または`teaser/index.html`）
- フォーム送信設定（Formspree等）
...
| Week 2 | ステップ3: 利用マニュアル実装 | 0.5日 | ✅ 完了 | 短時間で完了可能（2026-01-17完了） |
| Week 2 | ステップ4: ティザーサイト作成・公開 | 0.5日 | 未着手 | 2026年2月公開予定 |
| Week 2 | ステップ5: 開発者管理ページ（1日目） | 1日 | 未着手 | データベース・Backend API |
| Week 2 | ステップ5: 開発者管理ページ（2日目） | 1日 | 未着手 | Frontend実装 |
...
- **ステップ3（利用マニュアル）**: ステップ1のFrontend実装と並行可能（✅ 完了）
- **ステップ4（ティザーサイト）**: ステップ1のFrontend実装と並行可能（独立したHTMLページのため）
- **ステップ5（開発者管理ページ）**: ステップ1のBackend実装と並行可能
...
- ✅ 統合ヘルプシステム実装完了（2025-01-14完了）
  - ローカル環境: 完了
  - ステージング環境デプロイ: 完了（2025-01-14）
  - ステージング環境初期データ投入: 完了（2025-01-14）
  - 残存課題: FAQ内容修正、FAQ関連URL修正
- ✅ 月次ダッシュボード統計実装完了（2026-01-16完了）
- ✅ 利用マニュアル実装完了（2026-01-17完了）
- ⏳ ティザーサイト作成・公開（2026年2月公開予定）
- ⏳ 開発者管理ページ実装完了（オプション）
- ✅ 統合ヘルプシステムのテストがパスする（2025-01-14完了）
- ✅ 統合ヘルプシステムのブラウザテスト完了（2025-01-14完了）
- ✅ 統合ヘルプシステムのステージング環境動作確認完了（2025-01-14完了）
...
- ✅ ブラウザテスト完了（Chrome, Safari）（2026-01-17完了）

**ステップ4（ティザーサイト）**:
- ティザーサイトが正常に表示される（ファーストビュー、対象宿泊施設の明示、サービスの考え方、登録フォーム）
- レスポンシブ対応が完了している（モバイル・タブレット・デスクトップ）
- フォーム送信が正常に動作する（Formspree等）
- ブラウザテスト完了（Chrome, Safari, Firefox, Edge）
- デプロイ完了（GitHub Pages等）
- SEO設定完了（noindex、description）
- 「売り込まれた」と感じさせていない
- 「現場を分かっていそう」と思われている
...
- **FAQ関連URLの修正**: 統合ヘルプシステムのFAQデータ内の「該当ページへ移動」リンク（`related_url`）に、存在しないページや間違ったURLが含まれている。すべてのFAQの`related_url`を確認し、正しいURLに修正が必要（2025-01-14 ステージング環境確認時に発見）

#### 新規発見課題（2026-01-17 セッション）

**1. 月次統計の期間ロジックとStripe統合の整合性問題** ✅ **完了（2026-01-17）**

**問題点**:
- 現在の実装はカレンダー月（毎月1日〜月末）で集計しているが、Stripeの請求期間は契約開始日から1ヶ月
- `plan_started_at`フィールドは存在するが未使用
- Stripe実装を見据えると、契約開始日ベースの請求期間での集計に変更が必要

**影響範囲**:
- `get_monthly_usage`メソッド（今月の質問数）
- `get_ai_automation`メソッド（今月のAI自動応答数）
- `get_escalations_summary`メソッド（今月のエスカレーション数）
- `get_unresolved_escalations`メソッド（未解決のエスカレーション）

**優先度**: 高（Stripe統合前に解決必須）

**完了内容**:
- ✅ 設計書の更新完了（`docs/monthly_dashboard_arch.md`）
- ✅ 請求期間計算ユーティリティ関数の作成完了（`backend/app/utils/billing_period.py`）
- ✅ Backend API修正完了（`backend/app/services/dashboard_service.py`）
- ✅ フロントエンド表示ラベルの調整完了
- ✅ テスト実装完了（ユニットテスト・統合テスト）
- ✅ ブラウザテスト完了（ローカル環境・ステージング環境）
- ✅ CI/CD環境整備完了
- ✅ ステージング環境デプロイ完了・動作確認完了

**参照文書**: `docs/20260117_月次統計期間ロジックStripe統合整合性問題_調査分析_修正ステップ計画.md`

---

**2. FAQ登録時の言語数制限バリデーション未実装** - ✅ **完了（2026-01-17）**
...
**3. ゲスト側の言語選択画面でプランに応じた言語フィルタリング未実装** - ✅ **完了（2026-01-17）**
   - 実装完了、テスト完了、デプロイ完了
   - 追加で解決した課題: 日本語未表示問題、Premiumプラン言語リスト修正、言語選択後のページ表示修正、セッションID重複エラー修正、datetime比較エラー修正、重複送信防止、チャットAPIタイムアウト設定延長
   - コミット: `4aa65d5`, `e806a41`, `ef31b8a`
...
**実装完了内容**:
- Backend: 施設情報取得APIに`plan_type`と`available_languages`を追加
- Backend: 選択した言語の翻訳を優先的に取得するように修正
- Backend: Premiumプランの言語リストをStandardプラン＋韓国語に修正
- Backend: セッションIDの重複エラーハンドリングを追加
- Frontend: `SUPPORTED_LANGUAGES`に日本語、中国語、フランス語などを追加
- Frontend: プランに応じた言語フィルタリングを実装
- Frontend: Welcome画面で選択した言語をAPIリクエストに含める
- Test: プラン別の言語取得テストを追加

**コミット**: `4aa65d5`（`develop`ブランチ）
...
**問題点**:
- エスカレーションされた質問に対して、管理者が直接返信する機能が未実装
- 利用マニュアルでは「将来機能」として記載
...
**5. 利用マニュアルの目次UX問題** ✅ **完了（2026-01-18）**

**問題点**:
- 目次の章タイトルをタップすると、折りたたみ/展開したいだけなのにリンク先にジャンプしてしまう
- 開いている詳細を見ようと思ってタップしてもリンク先にジャンプして見ることができない
- ユーザー体験を著しく下げている
...
**6. 利用マニュアルのトップへ戻るボタン未実装**

**問題点**:
- 長いマニュアルをスクロールした後、トップに戻るボタンがない
- ユーザビリティが低下している
...
1. **ステップ1: 統合ヘルプシステム**（最優先、7.5日）✅ 完了
2. **ステップ2: 月次ダッシュボード統計**（最優先、2-3日）✅ 完了
   - 施設管理者のコスト意識に対応、最重要
   - ステップ1と並行実装推奨
3. **ステップ3: 利用マニュアル**（中優先、3-4時間）✅ 完了
4. **ステップ4: ティザーサイト作成・公開**（中優先、2-3時間）
   - PoC進行中〜正式ローンチ直前（2026年2月〜4月初旬公開予定）
5. **ステップ5: 開発者管理ページ**（中優先、10-14時間）

### 9.2 推奨実装順序

1. **ステップ1とステップ2を並行実装**（最優先）✅ 完了
   - ステップ1: 統合ヘルプシステム（ビジネス価値が高い）
   - ステップ2: 月次ダッシュボード統計（施設管理者のコスト意識に対応、最重要）
   - 両方とも最優先だが、ステップ2は施設管理者の満足度に直結するため、並行実装を推奨
2. **ステップ3を実装**（実装時間が短い）✅ 完了
3. **ステップ4を実装**（2026年2月公開予定）
   - PoC進行中に関心を持った宿泊施設に「あとで正式に確認したい」という保留フラグを立ててもらう
   - 本番LP公開前に、思想・方向性を正しく伝える
...
**Status**: ✅ **ステップ1（統合ヘルプシステム）実装完了、ステージング環境デプロイ完了、初期データ投入完了、残存課題記録済み**
...
**課題1: 月次統計の期間ロジックとStripe統合の整合性問題** ✅ **完了（2026-01-17）**

**実装ステップ**:

1. **設計書の更新**（30分） ✅ **完了**
   - `docs/monthly_dashboard_arch.md`を更新
   - カレンダー月ベースから契約開始日ベースの請求期間に変更
   - Stripe統合を見据えた設計に更新

2. **請求期間計算ユーティリティ関数の作成**（30分） ✅ **完了**
   - `backend/app/utils/billing_period.py`を新規作成
   - `calculate_billing_period`関数を実装
   - `python-dateutil`の`relativedelta`を使用して正確な月次計算を実装

3. **Backend API修正**（2-3時間） ✅ **完了**
   - `backend/app/services/dashboard_service.py`の以下のメソッドを修正:
     - `get_monthly_usage`: `plan_started_at`を基準とした請求期間で集計
     - `get_ai_automation`: 同上
     - `get_escalations_summary`: 同上
   - 請求期間計算ユーティリティ関数を使用して実装

4. **フロントエンド表示の調整**（30分） ✅ **完了**
   - 月次統計カードの表示ラベルを「今月」から「請求期間」に変更
   - 「利用状況」セクションのサブタイトルを「請求期間の統計とコスト情報」に変更

5. **テスト実装**（1-2時間） ✅ **完了**
   - 請求期間ベースの集計ロジックのユニットテスト（`backend/tests/test_billing_period.py`）
   - 統合テスト（`backend/tests/test_dashboard_service_billing_period.py`）
   - 境界値テスト（請求期間の開始日・終了日）
   - 複数の請求期間にまたがるデータのテスト
...
6. **ブラウザテスト**（30分） ✅ **完了**
   - ローカル環境でのブラウザテスト完了
   - ステージング環境でのブラウザテスト完了
   - 請求期間ベースの集計が正しく表示されることを確認

7. **CI/CD環境整備**（30分） ✅ **完了**
   - テストを必須化（`continue-on-error`を削除）
   - テストカバレッジの測定を追加
   - テスト結果のレポート生成とアップロードを追加

**所要時間**: 約5-7時間（1日）  
**実際の所要時間**: 約1日（2026-01-17）  
**参照文書**: 
- `docs/monthly_dashboard_arch.md`
- `docs/20260117_月次統計期間ロジックStripe統合整合性問題_調査分析_修正ステップ計画.md`
...
**課題2: FAQ登録時の言語数制限バリデーション実装** - ✅ **完了（2026-01-17）**

**実装ステップ**:

1. **Backend API修正**（1-2時間） ✅ **完了**
   - `backend/app/services/faq_service.py`の`create_faq`メソッドにバリデーション追加
   - `backend/app/services/faq_service.py`の`update_faq`メソッドにバリデーション追加
   - `_get_facility_languages`ヘルパー関数を追加

2. **エラーメッセージの追加**（30分） ✅ **完了**
   - `backend/app/core/error_messages.py`に`LANGUAGE_LIMIT_EXCEEDED`エラーメッセージを追加
   - 多言語対応（日本語・英語）

3. **フロントエンドエラーハンドリング**（30分） ✅ **完了**
   - `frontend/src/views/admin/FaqManagement.vue`で言語制限エラーを適切に表示

4. **テスト実装**（1時間） ✅ **完了**
   - ユニットテスト8ケース実装完了（`backend/tests/test_faq_service_language_limit.py`）
   - APIテスト5ケース実装完了（`backend/test_faq_language_limit_api.py`）
   - プラン別の言語制限テスト完了

5. **ブラウザテスト**（30分） ✅ **完了**
   - Freeプラン（1言語制限）: 新しい言語追加時にエラー表示確認
   - Miniプラン（2言語制限）: 新しい言語追加時にエラー表示確認

6. **バグ修正** ✅ **完了**
   - `backend/app/services/auth_service.py`の`get_plan_defaults`関数の`language_limit`バグを修正
   - 既存施設の`language_limit`修正スクリプト作成（`backend/fix_staging_language_limits.py`）
...
- `frontend/src/views/admin/FaqManagement.vue`: エラーハンドリング
- `backend/tests/test_faq_service_language_limit.py`: ユニットテスト
- `backend/test_faq_language_limit_api.py`: APIテスト
- `backend/fix_staging_language_limits.py`: データベース修正スクリプト

**コミット**: `2a43c26`（`develop`ブランチ）

---

**課題3: ゲスト側の言語選択画面でプランに応じた言語フィルタリング実装**

**実装ステップ**:

1. **Backend API追加**（1時間）
   - `backend/app/api/v1/chat.py`または`backend/app/api/v1/guest.py`に、施設の利用可能言語を取得するエンドポイントを追加:
     ```python
     @router.get("/available-languages")
     async def get_available_languages(facility_id: int):
         facility = get_facility(facility_id)
         plan_limits = get_plan_limits(facility.plan_type)
         return {"languages": plan_limits['languages']}
     ```

2. **フロントエンド修正**（1-2時間）
   - `frontend/src/views/guest/LanguageSelect.vue`を修正:
     - 施設IDから利用可能言語を取得
     - `SUPPORTED_LANGUAGES`の代わりに、取得した言語リストを表示
     - 言語選択時に、選択した言語が利用可能かチェック
...
4. **ブラウザテスト**（30分）
   - プランに応じた言語が正しく表示されることを確認

**所要時間**: 約3-4時間（0.5日）  
**参照文書**: `backend/app/core/plan_limits.py`  
**実装ファイル**: 
- `backend/app/api/v1/chat.py`（または新規エンドポイント）
- `frontend/src/views/guest/LanguageSelect.vue`

---

**課題5: 利用マニュアルの目次UX改善** ✅ **完了（2026-01-18）**

**実装ステップ**:

1. **目次コンポーネントの修正**（1-2時間） ✅
   - `frontend/src/components/admin/ManualToc.vue`を修正:
     - 章タイトルに折りたたみ/展開用のアイコンボタンを追加
     - 章タイトルのクリックイベントを分離:
       - アイコンボタン: 折りたたみ/展開のみ
       - 章タイトルテキスト: リンク先にジャンプ

2. **アクティブ状態表示機能の削除**（追加修正） ✅
   - `ManualToc.vue`: `activeSection`プロップと`active`クラスを削除
   - `ManualContent.vue`: `IntersectionObserver`と`section-change`イベントを削除
   - `Manual.vue`: `activeSection`の管理と`handleSectionChange`関数を削除
   - 約70行のコード削減により、シンプルで保守しやすい実装に変更
...
**課題6: 利用マニュアルのトップへ戻るボタン実装**

**実装ステップ**:

1. **トップへ戻るボタンコンポーネント作成**（1時間）
   - `frontend/src/components/admin/ManualScrollToTop.vue`を作成:
     - スクロール位置が一定以上（例: 300px）になったら表示
     - クリックでページトップにスムーススクロール
     - 固定位置（右下など）に配置

2. **Manual.vueへの統合**（30分）
   - `Manual.vue`に`ManualScrollToTop`コンポーネントを追加

3. **スタイリング**（30分）
   - ボタンのデザイン（Tailwind CSS）
   - ダークモード対応
   - アニメーション（フェードイン/アウト）

4. **ブラウザテスト**（30分）
   - スクロール時の表示/非表示が正しく動作することを確認
   - クリックでトップに戻ることを確認

**所要時間**: 約2時間（0.25日）  
**実装ファイル**: 
- `frontend/src/components/admin/ManualScrollToTop.vue`（新規）
- `frontend/src/views/admin/Manual.vue`

---

**課題4: 手動返信機能実装**（Phase 3以降）

**実装ステップ**: Phase 3以降で実装予定

1. **データベース設計**（1時間）
   - `manual_replies`テーブル作成
   - `conversations`テーブルとの関連付け

2. **Backend API実装**（3-4時間）
   - 手動返信の送信API
   - 返信履歴の取得API
...
1. **Week 1（優先度: 高）**
   - 課題1: 月次統計の期間ロジック修正（1日）
   - 課題2: FAQ登録時の言語数制限バリデーション（0.5日）
   - 課題3: ゲスト側の言語選択画面修正（0.5日）

2. **Week 2（優先度: 中）**
   - 課題5: 利用マニュアルの目次UX改善（0.5日）
   - 課題6: 利用マニュアルのトップへ戻るボタン（0.25日）

3. **Phase 3以降（優先度: 低）**
   - 課題4: 手動返信機能実装（1.5-2日）

**合計所要時間**: 約3-4日（優先度: 高・中のみ）
...
- Stripe統合前に必ず実装すること
- 既存のデータとの整合性を確認すること
- 請求期間の境界値（開始日・終了日）の処理に注意すること

**言語制限バリデーション**:
- 既存のFAQデータとの整合性を確認すること
- プラン変更時の既存FAQの扱いを検討すること

**目次UX改善**:
- モバイルでの動作を必ず確認すること
- アクセシビリティ（キーボード操作、スクリーンリーダー対応）を考慮すること
