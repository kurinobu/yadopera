...
1. **目次コンポーネントの修正**（1-2時間）
   - `frontend/src/components/admin/ManualToc.vue`を修正:
     ...
     - 章タイトルのクリックイベントを分離:
       - アイコンボタン: 折りたたみ/展開のみ
       - 章タイトルテキスト: リンク先にジャンプ
     - または、章タイトルを右クリックで折りたたみ/展開、左クリックでジャンプ（UXが複雑になる可能性あり）

2. **UX改善案の検討**（30分）
   - アイコンボタン方式（推奨）: 章タイトルの左側に`▼`/`▶`アイコンを配置
   - ホバー時の動作: 章タイトルにホバーすると折りたたみ/展開ボタンが表示される
...
**所要時間**: 約2-3時間（0.5日）  
**実装ファイル**: `frontend/src/components/admin/ManualToc.vue`
