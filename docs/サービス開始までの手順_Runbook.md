...
**作成日**: 2026年3月19日  
**最終更新日**: 2026年3月29日（§3.4.1.1 ステージング Webhook アカウント整合・引き継ぎ `docs/20260329_セッション引き継ぎ_Stripe_Webhook_ステージング.md`）  
...
**ステージング §6.5（ブラウザ・Stripe test、チェックリスト 1〜6）の証跡正本**: `docs/20260328_サービス開始ロードマップ_実行記録.md`（2026-03-29〜30 完走・✅ 記録）。
...
**5 / 5 — ステージング Stripe**: コマンド1本では不可。**`docs/サービス開始までの手順整理_実装計画.md` の `### 6.5 必須シナリオ（ステージング・Stripe test mode）`** のチェックリストを手で打勾。（**`docs/20260327_…調査修正案.md` の §6.5「フェーズ依存」とは別**。）**実施・証跡の記入例（完走済み）**: `docs/20260328_サービス開始ロードマップ_実行記録.md` の §6.5 表。
...
**コピペ1本では完結しない**（URL・アカウント・Stripe test mode）。**`docs/サービス開始までの手順整理_実装計画.md` §6.5** の項目を**そのままチェックリスト化し、実行した行に日付と結果を書く**。**記入例（完走）**: `docs/20260328_サービス開始ロードマップ_実行記録.md` §6.5 表。  
...
- §1.4（品質ゲート）は `docs/20260323_Runbook1.4_品質ゲート進行記録.md` で完了記録済み（**ステージング §6.5 ブラウザ証跡**の正本は `docs/20260328_サービス開始ロードマップ_実行記録.md`、2026-03-29〜30）。  
- §2.2（本番DBバックアップ）は完了。  
...
#### 3.4.1.1 ステージング専用: `STRIPE_SECRET_KEY` と Webhook 署名は **同一 Stripe アカウント** であること（必須）

**背景（2026-03-29 事実）**: Render の `STRIPE_SECRET_KEY`（`sk_test_51...`）が属する **アカウント ID** と、**`STRIPE_WEBHOOK_SECRET`（`whsec_...`）をコピーした Webhook Endpoint が表示されているダッシュボードの URL の `acct_...`** が **一致しない**と、Webhook は **POST 届くが 400（署名検証失敗）**のままになる。別アカウントの Endpoint の `whsec` を貼っても直らない。

**確認手順（画面ベース）**

1. **Render** → 対象サービス（例: `yadopera-backend-staging`）→ **Environment** → **`STRIPE_SECRET_KEY`** を表示する。
2. 値が `sk_test_51` で始まる場合、その直後の断片（例: `T4aV5LnkMufdVqu`）は **Stripe のアカウント ID `acct_1` + 同一断片**（例: `acct_1T4aV5LnkMufdVqu`）に対応する。
3. **Stripe** で **同一の `acct_...`** が URL に含まれる画面を開く。  
   - Webhook 一覧（ワークベンチ）の例:  
     `https://dashboard.stripe.com/acct_<上記と同じ>/test/workbench/webhooks`  
   - **顧客**一覧から飛ぶ場合: アドレスバーの `/test/customers` を `/test/workbench/webhooks` に置き換えてもよい（**`acct_...` は変えない**）。
4. 送信先 URL が **`https://<staging-backend>/api/v1/webhooks/stripe`** の行を開き、**署名シークレット（`whsec_...`）だけ**をコピーする → Render の **`STRIPE_WEBHOOK_SECRET`** に貼る（**`STRIPE_SECRET_KEY` には貼らない**）。
5. **Save** 後 **デプロイ／再起動完了**を待つ。

**検証**: 環境変数変更は **過去のログ行を書き換えない**。**新しい** `POST .../webhooks/stripe` が必要。  
- **Stripe ワークベンチ → イベント**で対象イベントを開き、**「Webhook エンドポイントへの配信」に「再送」がある場合のみ**押す（**画面にボタンが無ければ無理に探さない**。別メールでの **新規登録（有料）1 回**でも Webhook は再発火する）。  
- **Render Logs** で `webhooks/stripe` を検索し、**修正後の時刻の POST が 200** か確認する。

**詳細な経緯・別 `acct` 混在例**: `docs/20260329_セッション引き継ぎ_Stripe_Webhook_ステージング.md`。
...
- [x] DBマイグレーション失敗、または不整合発生。  
  - 該当なし（§2.4・§9.2 で本番 `alembic_version=023`（Free `faq_limit` 整合）まで確認済み。以降のデプロイ後は **コード SHA と DB revision の両方**で再確認すること）。
...
記録に残すとよい一行（コピペ用）
日付: 2026-03-28（メニューバーに合わせてよい）
URL: http://localhost:5173/admin/faqs
ログイン: test12@air-edison.com
表示: 「現在 28 / 上限 100 件」あり（§9.3 ステップ 2 満たす）
環境: Docker ローカル（5173 / 手順 4 実行後）
※ 「100」はその施設のプランに応じた上限表示で、Free の「30」と違っていてもおかしくありません。§9.3は 件数行の有無と読み方の確認が主目的です。
...
**Status**: A（FAQ上限制御）を本番まで完了。`main` 反映後に本番DBの Alembic `022 -> 023` を適用し、Free の `faq_limit=30` を確認。ステージング・本番ともに 31件目拒否が「30件まで」で一致。**§9.3**・**§1.4.1**（§6.0 ローカル 1〜5 のコピペ・要否）を文書化済み。**§1.4**: 進行記録に加え、**実装計画 §6.5**（ステージング Stripe 1〜6）は `docs/20260328_サービス開始ロードマップ_実行記録.md` で 2026-03-29〜30 に証跡付き完走（残存課題は同書「残存課題」参照）。  
**2026-03-29（フェーズ D・ステージング）**: `develop` コミット **`6a48784`** をプッシュしステージングへデプロイ。**実行コミット確認**: `GET https://yadopera-backend-staging.onrender.com/__debug_env` の **`RENDER_GIT_COMMIT`** が上記と一致。**マニュアル D1**: **`https://yadopera-frontend-staging.onrender.com/admin/manual`** → 左 **ご利用マニュアル**（当該 URL）→ 目次 **第4章 → 4.2 FAQ一覧の見方** で文言確認（FAQ管理 `/admin/faqs` とは別画面）。**調査修正案** `docs/20260327_本番検証_チェック結果と課題_調査修正案.md` の **§6 フェーズ A〜D は文書上完了**。**次の作業の正本**: 本 Runbook **§2 以降**の未了項目と **`docs/サービス開始までの手順整理_実装計画.md` §6** の **段3以降**（本番インフラ・Stripe live 等）。§6.5 ステージング必須シナリオの再実行はリグレッション時。
