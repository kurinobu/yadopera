...
├── Response (201 Created)
│   ├── qr_code_id: integer
│   ├── url: string (https://tabipera.com/f/{facility_id}?location={location})
...
│
├── CORS Middleware
│   ├── allow_origins: ["https://tabipera.com", "https://admin.tabipera.com"]
...
CREATE TABLE qr_codes (
    ...
    url TEXT NOT NULL,  -- https://tabipera.com/f/{facility_slug}?location={location}
...
管理画面で削除できます:
https://admin.tabipera.com/admin/data-cleanup
...
app.add_middleware(
    ...
    allowed_hosts=["tabipera.com", "admin.tabipera.com", "*.tabipera.com"]
)
...
管理画面で確認・対応してください:
https://admin.tabipera.com/admin/escalations/{escalation.id}
"""
...
SMTP_USER=noreply@tabipera.com
SMTP_PASSWORD=xxxxx
FRONTEND_URL=https://tabipera.com  # v0.2追加
...
# CORS
CORS_ORIGINS=https://tabipera.com,https://admin.tabipera.com
...
SMTP_USER=...
SMTP_PASSWORD=...
SMTP_FROM_EMAIL=noreply@tabipera.com
...
2. **簡易ランディングページ（LP）作成**★新規
   ...
   - **技術仕様**:
     - 静的HTML/CSS/JavaScript（またはVite + Vue.js 3）
     - レスポンシブデザイン（スマホ最適化）
     - フォーム送信: 外部サービス（Google Forms、Typeform等）または簡易API
     - デプロイ: Netlify、Vercel、GitHub Pages等の静的ホスティング
   - **工数**: 数時間（1ページの静的LP）
   - **デモ動画内容**:
     - QRコード読み取り
     - 多言語選択
