...
- **Phase B（埋め込み事前計算）**: **✅ 実装・テスト・手順整備 完了（2026-03-14）**。
  - **B1〜B8**: スキーマ拡張・定数・ローダー・スクリプト・auth_service・create_faq の変更を実施。B6 にて事前計算 JSON（210件）を生成・コミット（別環境で push 済みの場合はリモートに反映済み）。
  - **B9**: ローダー単体3件＋create_faq 経路（事前計算時は `generate_embedding` 未呼出）1件の計4テストを追加。Docker 上でパス（PostgreSQL 使用時は4件とも実行、SQLite 時は create_faq テストはスキップ）。
  - **B10**: 手順書 `docs/新規登録時_自動登録FAQ_B10_デプロイとステージング確認手順.md` を整備。**ステージングでの実確認（新規登録→30件投入・RAG動作・ログで埋め込みAPI未使用）は手順書に従い実施すること。未実施の場合は Phase B 完了前に実施する。**
  - **B11**: 本節（Status・実施結果・運用時の参照）の文書更新で完了。

**Phase B を「完了」とするためのチェックリスト**

| 項目 | 状態 |
|------|------|
| B1〜B8 のコードが develop にコミット・push されている | 要確認（未コミットの場合はコミット・push すること） |
| B6 で生成した `faq_presets_embeddings.json` がリポジトリに含まれている | 要確認 |
| B9 のテストファイルがコミットされている | 要確認 |
| B10 手順書に従い、ステージングで新規登録→30件・RAG・ログ確認を実施した | 要実施（未実施なら実施すること） |
| B11 文書更新（本ドキュメント）がコミットされている | 本更新で完了 |
...
| B10 | デプロイ・ステージング確認 | 手順書を整備。実確認は手順書に従い実施。 |
| B11 | 文書更新 | 本ドキュメントの Status・実施結果・運用時の参照を更新。 |
