...
20. **料金プラン詳細化**: 
    - Mini: ¥1,980/月 + ¥30/質問（最初から従量課金） FAQ登録数20
    - Small: ¥3,980/月、200件/月まで（超過分は+ ¥30/質問） FAQ登録数50
    - Standard: ¥5,980/月、500件/月まで（超過分は+ ¥30/質問） FAQ登録数100
    - Premium: ¥7,980/月、1,000件/月まで（超過分は+ ¥30/質問）FAQ登録数無制限
21. **月次サマリー統計カード仕様追加**: 
...
- **宿泊施設管理者向けFAQ実装**★新規
  ...
  - 管理画面内にFAQページを追加
  - よくある質問と回答を提供
- **Phase 2残存課題対応**★新規（2025-12-21追加）
  - **セッション有効期限延長問題の対応**: 防止策1（`started_at`ベースの固定有効期限）を実装。チェックアウト後もアクセス可能になる問題を防止
  - **宿泊管理者ダッシュボードグラフ削除問題の対応**: グラフコンポーネント（`CategoryChart`）のみをコメントアウト（マスク）。統計カード（スコア）は表示を維持
  - **PWAインストールプロンプトのボタンの動作問題の対応**: スマホのホーム画面にアイコンが表示されるが、アイコンをタップすると404エラーが発生する問題を調査・修正。PWAインストール後のルーティング問題を解決
  - **ランディングページの料金プラン表示変更**: 新しい料金プラン（Mini/Small/Standard/Premium）を反映
...
**Document Version**: v0.3.5  
**Based on**: v0.21 要約定義書  
**Author**: Air  
**Last Updated**: 2025-12-21 15時30分00秒  
**Status**: 開発準備完了
