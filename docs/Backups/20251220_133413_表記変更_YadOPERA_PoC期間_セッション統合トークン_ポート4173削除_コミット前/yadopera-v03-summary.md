...
**セッション管理（改善版）**
- SessionID: Cookie保存
- **会話引き継ぎコード**: 4桁英数字（例: `AB12`）
  - 初回アクセス時に発行、画面上部に常時表示
  - 別デバイスでQRスキャン時、会話引き継ぎコード入力で会話履歴統合
  - `https://yadopera.com/f/{facility_id}?token=AB12`
- 有効期限: 無動作24時間で自動終了
- 複数デバイス: 会話引き継ぎコードによる統合可能
...
### PoC施設数削減の理由
- 一人開発での品質担保
- 週次MTG工数: 3施設×30分×8週 = 12時間（旧: 30-42時間）
- 深いフィードバック収集が可能
- 成功パターン確立後にスケール
