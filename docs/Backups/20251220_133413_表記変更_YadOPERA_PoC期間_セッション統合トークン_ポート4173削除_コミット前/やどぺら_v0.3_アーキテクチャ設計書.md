# YadOPERA v0.3 アーキテクチャ設計書

**作成日**: 2025年11月21日  
**最終更新日**: 2025年12月20日 12時33分  
**バージョン**: v0.3.2 (v0.3.1からのバージョンアップ)  
**対象**: YadOPERA - 小規模宿泊施設向け外国人ゲスト対応自動化SaaS  
...
5. **会話引き継ぎコード**: 4桁コードでデバイス間会話履歴を統合
...
**v0.3実装詳細化（開発開始前明確化）**
13. **会話引き継ぎコード実装詳細**: 4桁英数字ランダム生成、重複チェック、試行回数制限、セキュリティ対策
...
#### session_tokens (会話引き継ぎコード) - v0.3新規
...
async def send_cleanup_alert(
    ...
    if count > 0:
        ...
        subject = f"[YadOPERA] データ削除のご案内: {facility.name}"
...
### 10.7 会話引き継ぎコードのセキュリティ対策（v0.3新規）
...
async def send_escalation_notification(
    ...
    エスカレーションメール通知（v0.2）
    """
    subject = f"[YadOPERA] エスカレーション: {facility.name}"
...
- 利用マニュアル作成
- 専用Slackチャンネル作成
- **宿泊事業者向けAIヘルプチャット実装**（実装時期変更: Expansion Phase A → Phase 2）
- **開発者管理ページ実装**（v0.3.1追加）
  - 契約宿泊管理者のエラーやスコアなどが閲覧できる統括ページ
- **デモサイト実装**（v0.3.1追加）
  - PoC契約に向け、参加希望者が閲覧できるデモサイト（テスト用IDとパスワード）
- **宿泊施設管理者向けFAQ実装**（v0.3.1追加）
