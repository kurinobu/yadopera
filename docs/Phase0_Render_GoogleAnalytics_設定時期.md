...
- **内容**:
  ...
  - ステージング環境変数設定
  - ステージング環境デプロイ確認
- **URL**: `https://staging.tabipera.com`（予定）
...
- **デプロイ先**: Vercel
- **カスタムドメイン**: `https://tabipera.com`（または`https://www.tabipera.com`）
...
2. **Vercelデプロイ後**:
   - データストリームのURLを`https://tabipera.com`に更新
   - 測定IDをHTMLに設定
...
カスタムドメイン設定（tabipera.com）
  ↓
Google Analytics 4プロパティ作成（URL: https://tabipera.com）
  ↓
測定ID取得
  ↓
HTMLに測定ID設定
...
【すぐに実施可能】Vercelにランディングページをデプロイ
  ↓
【すぐに実施可能】カスタムドメイン設定（tabipera.com）
  ↓
