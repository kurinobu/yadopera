...
class DashboardService:
    ...
    async def get_dashboard_data(self, facility_id: int) -> DashboardResponse:
        ...
        """
        # 週次サマリー取得
        summary = await self.get_weekly_summary(facility_id)
        
        # リアルタイムチャット履歴取得
        recent_conversations = await self.get_recent_chat_history(facility_id)
        
        # 夜間対応キュー取得
        overnight_queue = await self.get_overnight_queue(facility_id)
        
        # フィードバック統計取得
        feedback_stats = await self.get_feedback_stats(facility_id)
