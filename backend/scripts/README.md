...
**用途**: 指定施設にCSVからFAQを一括登録する（有料代行サービス履行用）。施設に紐づく1ユーザーを `uploaded_by` として使用。
...
- **ローカル**（`DATABASE_URL` 等が設定されている場合）
  ```bash
  cd backend
  python scripts/bulk_upload_faq_csv_for_facility.py --facility-id <施設ID> --csv /path/to/faq.csv
  ```
