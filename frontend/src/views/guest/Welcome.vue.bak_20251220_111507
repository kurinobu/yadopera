...
onMounted(async () => {
  try {
    ...
    console.error('[Welcome.vue] isOffline.value:', isOffline.value)
    
    if (errorCode === 'NETWORK_ERROR' || String(errorCode) === 'NETWORK_ERROR') {
      if (isOffline.value) {
        // オフライン時
        error.value = '現在オフラインです。インターネット接続を確認してください。'
        console.log('[Welcome.vue] NETWORK_ERROR検出 + オフライン状態: オフラインメッセージを表示')
      } else {
        // オンラインだがネットワークエラー
        error.value = 'ネットワークエラーが発生しました。接続を確認して再度お試しください。'
        console.log('[Welcome.vue] NETWORK_ERROR検出 + オンライン状態: ネットワークエラーメッセージを表示')
      }
