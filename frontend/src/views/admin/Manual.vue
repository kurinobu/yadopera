...
2. **★ FAQの登録（最重要）**（第4章参照）
   <span class="manual-purpose">登録数が多く、内容が具体的なほどAIの回答精度が上がり、自動応答率が高まります。初期デフォルトを確認し、施設に合わせて追加・編集してください。</span>
   - 30件の初期デフォルトFAQを確認して編集・削除・追加する
   - 基本情報のFAQ（チェックイン/アウト時間、WiFi情報など）を確認・編集する
   - 設備・サービスのFAQ（キッチン、シャワー、洗濯機など）を確認・編集する
   - 場所・アクセスのFAQ（最寄り駅、コンビニ、レストランなど）を確認・編集する
   - トラブル対応のFAQ（鍵紛失、WiFi不通、設備故障など）を確認・編集する
   - 各FAQの優先度を適切に設定する（よく聞かれる質問ほど高く）

3. **★ QRコードの発行（最重要）**（第8章参照）
   <span class="manual-purpose">ゲストが質問にアクセスする入口です。掲示・貼り付けに加え、チェックイン時にQRコードを読み込んでもらうと利用率が上がります。入口・客室など見やすい場所へ設置し、読み取りテストをしてください。</span>
   - 各設置場所（入口、客室、キッチン、ラウンジなど）のQRコードを生成する
   - QRコードを印刷して、適切な場所に設置する
   - チェックイン時にゲストにQRコードを読み込んでもらうと利用率が上がる
   - QRコードが正しく読み取れるかテストする
...
5. **初期設定チェックリストを確認**（第12章参照）
   - 第12章「運用のベストプラクティス」の「12.1 初期設定チェックリスト」を参照して、すべての項目が完了しているか確認してください

**初期設定のポイント**:
- 初期設定は一度だけ行えば、その後は必要に応じて更新するだけです
- FAQは、ゲストがよく聞く質問から優先的に登録してください（登録数・内容の充実が自動応答率向上の要です）
- QRコードは、ゲストがアクセスしやすい場所に設置し、チェックイン時に読み込んでもらうと利用率が上がります`
      }
    ]
  },
  {
...
- **CSV一括登録**（FAQ管理の画面上部に出るボタン・案内）は、**Standard または Premium** を選んだ施設のみ利用できます（ログイン後に表示。**Free・Mini・Small では出ません**。非表示は不具合ではありません）`
      },
      {
        id: 'login-logout',
        title: '2.2 ログアウト',
...
**1. パスワードを忘れた**:
- ログイン画面の「パスワードを忘れた場合はこちら」から、パスワードを再設定できます
- 手順の詳細は「2.4 パスワードを忘れた場合」を参照してください
...
- 詳細な手順は「第11章 トラブルシューティング」の「11.3 ブラウザキャッシュのクリア方法」を参照してください

**5. それでもログインできない場合**:
- システム管理者にお問い合わせください
- 問い合わせ方法は「第11章 トラブルシューティング」の「11.4 問い合わせ方法」を参照してください`
      },
      {
        id: 'login-forgot-password',
        title: '2.4 パスワードを忘れた場合',
        content: `**パスワードリセットの手順**:
パスワードを忘れた場合は、ログイン画面からパスワードを再設定できます。

1. **ログイン画面**で「パスワードを忘れた場合はこちら」のリンクをクリックします
2. **パスワードリセット画面**で、登録時に使ったメールアドレスを入力します
3. 「リセット用メールを送信」ボタンをクリックします
4. 登録メールアドレスに届いた**リセット用メール**のリンクをクリックします（リンクの有効期限は1時間です）
5. **新しいパスワード**と**確認用パスワード**を入力し、「パスワードを変更」をクリックします
6. 完了後、ログイン画面で新しいパスワードを使ってログインしてください

**注意**:
- リセット用メールが届かない場合は、迷惑メールフォルダをご確認ください
- 同じメールアドレスへのリセット用メールの再送は、60秒以上あけてから行えます
- ログインできる状態の場合は、施設設定の「パスワード変更」セクションからもパスワードを変更できます（第6章 6.4 参照）`
      }
    ]
  },
  {
...
- **クーポン発行数**: 請求期間内のリード（クーポン取得）数（詳細は3.8、一覧は第6章6.6参照）
...
- この請求期間の区切りは、プラン・請求ページ（第7章）の請求履歴と同じです
...
- 最新の会話を素早く確認できます
- 問題のある会話を早期に発見できます
- 詳細は「第9章 会話詳細の確認」を参照してください`
      },
      {
        id: 'dashboard-coupon-leads',
        title: '3.8 クーポン発行数',
        content: `**表示内容**:
- ダッシュボードの「その他の統計」エリアに「クーポン発行数」カードが表示されます
- 現在の請求期間内に、ゲストがクーポン取得（メールアドレス登録）した件数の累計が表示されます

**見方**:
- 数値が大きいほど、リード（顧客接点）を獲得できていることを示します
- クーポン設定を有効にし、割引率を設定している施設のみ、ゲスト画面にクーポンのボタンが表示され、この数値が増えます

**リード一覧の確認**:
- 取得者のメールアドレス・名前・取得日時は、左メニュー「リード（クーポン取得）」から一覧で確認できます
- 詳細は「第6章 施設設定」の「6.6 リード（クーポン取得）一覧の確認」を参照してください`
      }
    ]
  },
  {
...
- **CSV一括登録**（画面上部）・**CSV一括登録代行（有料）**への案内: **Standard / Premium** 契約の施設のみ表示。**Free・Mini・Small では表示されません**（仕様。本番で出ない場合はプランを確認してください）
...
**カテゴリ見出しの件数と、画面上部の「現在 / 上限」（混同しない）**:
- **「Basic (8件)」のような表示**は、その **Basic カテゴリに属する FAQ（インテント）の件数だけ**です。Facilities / Location / Trouble など **他カテゴリの件数は含みません**。
- 画面上部の **「現在 X / 上限 Y 件」** の **Y** は、料金プランに応じた **全カテゴリ合計の FAQ 登録上限**（**インテント単位**）です。**Free プランは 30 件まで**（サービス仕様・対外と一致）。
- **「Basic 8件」＝施設全体の上限 8 件ではありません。** 全体が上限に近いかどうかは、**「現在 X / 上限 Y 件」** の **X** で判断してください。
...
**✅ 良い例と避けたい例（回答は具体的に）**:
- 回答は「はい」「いいえ」だけでなく、**場所・時間・方法**などゲストが知りたい情報を具体的に書くと、AIの回答精度が上がり、追質問やエスカレーションが減ります。
- **避けたい例**: Q: 洗濯機はありますか？ A: はい、あります。
- **良い例**: Q: 洗濯機はありますか？ A: はい、シャワー室の隣にあります。8:00から22:00までご利用できます。現金が必要です。
...
- ゲストからのフィードバックを参考に、FAQを改善してください`
      ...
      {
        id: 'faq-csv-bulk',
        title: '4.8 CSV一括登録（Standard・Premium）',
        content: `**対象プラン**:
- CSV一括登録は**Standardプラン**と**Premiumプラン**でのみ利用できます
- Free・Mini・Smallプランでは利用できません

**画面の開き方**:
- 左メニュー「FAQ管理」をクリックし、画面上部の「CSV一括登録」ボタンをクリックします

**使い方（手順）**:
- ステップ1: テンプレートを取得する（下記「テンプレートの入手」のいずれかからダウンロード）
- ステップ2: CSVを編集する（必須カラムを埋め、必要に応じて多言語列を追加。1行＝1件のFAQ）
- ステップ3: 保存形式を合わせる（UTF-8（BOM付き推奨）で保存。Excel等で編集した場合は「CSV UTF-8（カンマ区切り）」で保存し直す）
- ステップ4: アップロードする（FAQ管理画面で「CSV一括登録」ボタンからモーダルを開き、ファイルを選択して送信）
- ステップ5: 結果を確認する（成功時は件数が表示されます。FAQ一覧で登録内容を確認してください）

**登録の意味（上書きではなく新規追加）**:
- 現在の機能では**新規追加のみ**です。既存のFAQには一切上書きしません
- CSVの各行が、新しいFAQとして**追加**されます（既存FAQの更新・一括削除はできません）

**重複について（事前の準備）**:
- 同じ質問・カテゴリ（intent_key）が既に登録されている行がCSVに含まれていると、**エラーになりアップロード全体が失敗**します
- 同じ質問が既にFAQに存在する場合は、**あらかじめFAQ一覧からそのFAQを削除するか、CSVファイルからその行を削除してから**アップロードしてください。重複がある場合はエラーが出る可能性があります
...
- **Excel（Windows）**: テンプレートCSVをExcelで開く → 編集する →「ファイル」→「名前を付けて保存」→ ファイルの種類で「**CSV UTF-8（カンマ区切り）（.csv）**」を選んで保存
- **Excel（Mac）**: 「ファイル」→「別名で保存」→ 形式で「**CSV UTF-8（コンマ区切り）（.csv）**」を選択して保存
- **Googleスプレッドシート**: ファイルを開く →「ファイル」→「ダウンロード」→「**カンマ区切りの値（.csv、現在のシート）**」を選択。文字コードはUTF-8でダウンロードされます
- **Numbers（Mac）**: 編集後「ファイル」→「書き出す」→「CSV」を選択。テキストエンコーディングで「UTF-8」を選んで書き出し

**多言語翻訳の参考**:
- CSVに多言語を追加する際は、本マニュアル付録の [12.2 翻訳手引書（ChatGPTを使った翻訳）](#appendix-translation) をご参照ください
...
- **重複エラー**: 同じ intent_key（同じカテゴリ＋同じ質問）の行が既に登録されています。FAQ一覧で該当FAQを削除するか、CSVからその行を削除してから再アップロードしてください
- **必須カラム不足**: category, language_ja_question, language_ja_answer のいずれかが空です。該当行を修正し、保存し直してから再アップロードしてください
- **文字コード・文字化け**: 保存時に「UTF-8（BOM付き）」または「CSV UTF-8（カンマ区切り）」を選んで保存し直してください
- **件数超過**: プランのFAQ登録数上限（Standardは100件まで等）を超えています。既存FAQを削除するか、登録件数を減らしてください
- **言語数超過**: プランの同時利用言語数を超えています。CSVの言語列をプラン上限内に減らすか、プラン変更をご検討ください
- 上記以外のメッセージが表示された場合は、表示内容に従ってCSVを修正し、再度アップロードしてください

**有料代行をご希望の場合**:
- **料金の目安（税別）**: Standard 〜30件 ¥12,000、〜60件 ¥22,000、〜100件 ¥32,000。Premium 〜50件 ¥25,000、〜100件 ¥45,000、〜200件 ¥68,000、200件超は個別見積（¥90,000〜）。詳細は申し込みページの料金表をご確認ください。
- **料金に含まれる**: 日本語FAQの自然さ・文法確認、軽微な表現修正、AI翻訳＋品質確認、管理画面への登録作業、表示・動作チェック（初回）。
- **料金に含まれない**: FAQの新規企画・構成設計、内容の事実確認、大幅な書き直し・再編集、法的表現監修・翻訳証明。
- Standard・Premiumプランの施設では、FAQ管理画面の「CSV一括登録」ボタンの近くに「CSV一括登録代行（有料）をご希望の方はこちら」リンクが表示されます。左メニューの「CSV代行（有料）の申し込み」からも同じ申し込みページへ進めます。
...
- 申し込みは [CSV一括登録 代行（有料）の申し込み](/admin/csv-bulk-request) から行えます。料金表や詳細は申し込みページ・運営までお問い合わせください`
      }
    ]
  },
  {
...
**施設設定画面の構成**:
- **基本情報セクション**: 施設名、メールアドレス、電話番号、住所。「ゲスト画面にメールアドレスを表示する」をONにするとゲストのウェルカム画面に施設のメールアドレスが表示されます。OFFにすると表示されません。表示する場合は、ログイン用とは別の施設用メールアドレスを設定してください。
- **WiFi設定セクション**: WiFi SSID、WiFiパスワード
- **チェックイン/アウト時間セクション**: チェックイン時間、チェックアウト時間
- **館内ルール・周辺情報セクション**: 館内ルール、周辺情報
- **スタッフ不在時間帯セクション**: スタッフ不在時間帯の設定
- **クーポン設定（リード獲得）セクション**: クーポンON/OFF、割引率、クーポン文言、有効期限、公式サイトURL
...
- **施設名**: 必須項目、1-255文字以内
- **メールアドレス**: 必須項目、メール形式
- **ゲスト画面にメールアドレスを表示する**: ON/OFF。ONのときはゲストのウェルカム画面に表示されます。OFFのときは表示されず、電話やチャットでの問い合わせ導線のみになります。
- **電話番号**: 任意項目、最大50文字
- **住所**: 任意項目
...
- スイッチをONにしている場合、施設のメールアドレスとログイン用メールアドレスが同じだと保存できません
...
**保存時のバリデーション**:
- **施設名**: 必須項目、1-255文字以内
- **メールアドレス**: 必須項目、メール形式。「ゲストにメールを表示する」がONのときは、ログイン用メールアドレスとは別の施設用メールアドレスを設定する必要があります
...
- パスワード変更は、施設設定の保存とは独立して実行されます`
      ...
      {
        id: 'facility-coupon-settings',
        title: '6.5 クーポン設定と公式サイトURL',
        content: `**クーポン設定（リード獲得）とは**:
- ゲストのメールアドレスを取得し、クーポンを送付する機能の設定です
- 施設設定画面の「クーポン設定（リード獲得）」セクションで設定します

**設定項目**:
- **クーポン有効/無効**: ONにすると、ゲスト画面の固定フッターに「オトクなクーポン」などのボタンが表示されます。OFFの場合は表示されません
- **割引率（%）**: 5〜20など、施設が提供する割引率を入力します。有効かつ割引率を設定した施設のみ、ゲストにクーポンのボタンが表示されます
- **クーポン文言**: 施設が編集する説明文（任意）。ゲストへの案内や送付メールに反映されます
- **有効期限（発行日から何ヶ月）**: クーポンの有効期間を「発行日から何ヶ月」で設定します
- **公式サイトURL**: 任意項目。入力すると、クーポン送付メールにそのURLが記載され、ゲストが次回予約時に参照できます

**保存方法**:
- 上記を入力・変更したあと、施設設定画面下部の「保存」ボタンをクリックすると、他の施設設定と一緒に保存されます`
      },
      {
        id: 'facility-leads-list',
        title: '6.6 リード（クーポン取得）一覧の確認',
        content: `**リード一覧画面へのアクセス**:
- 左メニューから「リード（クーポン取得）」をクリックすると、リード一覧画面（\`/admin/leads\`）に遷移します

**一覧の表示内容**:
- クーポン取得（メールアドレス登録）したゲストの一覧が表示されます
- メールアドレス、名前、取得日時などの項目を確認できます

**CSVダウンロード**:
- 一覧画面からCSV形式でダウンロードできます
- ボタンまたはリンクをクリックすると、一覧データがCSVファイルとしてダウンロードされます
- 顧客名簿やメール配信リストの作成に活用できます`
      }
    ]
  },
  {
    ...
    subsections: [
      {
        id: 'plan-billing-overview',
        title: '7.1 プラン・請求ページの概要',
        content: `**プラン・請求ページとは**:
- 料金プランの確認・変更、解約、請求履歴・領収書をまとめて確認・操作できる画面です
- 左メニューから「プラン・請求」をクリックしてアクセスします（施設設定の下にあります）

**このページでできること**:
- **現在のプランの確認**: ご契約中のプラン名と月額を確認できます
- **プラン変更**: 別のプランへ変更する手続きができます
- **解約**: 有料プランから解約し、Free プランへ戻る手続きができます
- **請求履歴・領収書**: 過去の請求一覧を確認し、領収書を表示できます

**ご注意**:
- 決済システム（Stripe）が未設定の施設では、「プラン変更・解約は利用できません」と表示されます。その場合はプラン変更や解約のボタンは表示されません。`
      },
      {
        id: 'plan-billing-current-list',
        title: '7.2 現在のプランとプラン一覧の見方',
        content: `**現在のプランの確認**:
- ページ上部の「現在のプラン」ブロックに、現在のプラン名と月額（税込）が表示されます
- 例: 「Small（月額 3,980 円）」「Free」など

**プラン一覧の見方**:
- 「プラン一覧」の表で、利用可能なプラン（Free、Mini、Small、Standard、Premium）を一覧できます
- **プラン**: プラン名。現在ご契約中のプランには「（現在）」と表示されます
- **月額（税込）**: 各プランの月額。Free は「無料」と表示されます
- **月間質問数**: そのプランで含まれる月間の質問数（Mini は「従量」＝質問ごとの課金）
- **FAQ数**: 登録できるFAQの上限件数
- **言語数**: 同時に利用できる言語の数

**料金体系の概要**:
- **Free**: 1言語（日本語のみ）、無料、月30質問まで、FAQ 30件
- **Mini**: 2言語（日本語・英語）、¥1,980/月、質問は従量（¥30/質問）、FAQ 30件
- **Small**: 3言語（日本語・英語・繁体中国語）、¥3,980/月、200質問まで（超過分は¥30/質問）、FAQ 50件
- **Standard**: 4言語（日本語・英語・繁体中国語・フランス語）、¥5,980/月、500質問まで（超過分は¥30/質問）、FAQ 100件
- **Premium**: 言語無制限（日本語・英語・繁体中国語・簡体中国語・フランス語・韓国語・スペイン語）、¥7,980/月、1,000質問まで（超過分は¥30/質問）、FAQ 無制限

※「2言語」「3言語」は任意に好きな言語を選べる意味ではなく、プランごとに定義された対応言語セットです。`
      },
      {
        id: 'plan-billing-overage',
        title: '7.3 プラン超過時の挙動の設定',
        content: `**プラン超過時の挙動とは**:
- 月間の質問数がプランの上限（例: Free は30件、Small は200件）を超えたあと、ゲストが質問したときにどうするかを選べる設定です
- この設定は「プラン・請求」ページの「プラン超過時の挙動」セクションで変更できます（Free、Small、Standard、Premium のとき表示されます。Mini は質問数に上限がないためこの設定は表示されません）

**2つの選択肢**:
- **通常継続（従量課金）**: 上限を超えたあとも、これまでどおりAIが回答します。超えた分は1質問あたり¥30で請求されます。ダッシュボードの利用状況では「超過（従量）」と表示されます
- **AI停止・FAQのみ対応**: 上限を超えたあとは、AIは使わず、登録したFAQの検索結果だけで回答します。超えた分の課金はありません。ダッシュボードでは「FAQのみ対応中」のように表示されます

**設定の変更方法**:
- 希望の選択肢をラジオボタンで選び、「設定を保存」ボタンをクリックすると反映されます
- 変更後、ダッシュボードの月次利用状況表示も、選んだ内容に合わせて変わります`
      },
      {
        id: 'plan-billing-change',
        title: '7.4 プラン変更の手順',
        content: `**プラン変更ができる場合**:
- 決済システムが設定済みの施設で、現在と異なるプランへ変更する場合に「プラン変更」ボタンが表示されます
- 現在のプランと同じ行にはボタンは表示されません

**プラン変更の手順**:
1. プラン一覧で、変更したいプランの行にある「プラン変更」ボタンをクリックします
2. 「プラン変更の確認」というモーダル（小窓）が開きます
3. モーダルに表示されたプラン名と月額を確認します
4. 問題なければ「変更する」ボタンをクリックして実行します
5. 変更が完了すると、画面上の現在のプラン表示が更新されます

**変更後の注意**:
- プラン変更後も、既存のQRコード・FAQ・施設設定はそのまま利用できます
- アップグレードの場合は即時反映、ダウングレードも同様に手続き後反映されます`
      },
      {
        id: 'plan-billing-cancel',
        title: '7.5 解約の手順',
        content: `**解約ができる場合**:
- 有料プラン（Mini、Small、Standard、Premium）をご利用中で、かつ決済システムが設定済みの施設にのみ「解約」のブロックが表示されます
- Free プランの施設には解約の表示はありません

**解約の手順**:
1. 「解約する」ボタンをクリックします
2. 「解約の確認」モーダルが開きます
3. 解約方法を選択します:
   - **期間末で解約**: 現在の利用期間（請求期間）が終了したあとに Free プランへ移行します。それまでは有料プランのまま利用できます
   - **即時解約**: 手続き後、すぐに Free プランへ移行します
4. 選択後、「解約する」ボタンをクリックして手続きを完了します

**解約後**:
- 解約が完了すると Free プランになります。Free プランの制限（1言語、月30質問まで、FAQ 30件など）が適用されます
- 解約後に再度有料プランへ変更したい場合は、プラン・請求ページの「プラン変更」から希望のプランを選んで変更できます`
      },
      {
        id: 'plan-billing-invoices',
        title: '7.6 請求履歴と領収書',
        content: `**請求履歴の見方**:
- 「請求履歴・領収書」の表に、過去の請求が一覧表示されます
- **請求日**: 請求が発生した日付
- **金額**: 請求金額（円）
- **ステータス**: 支払い状況など
- 請求がまだ1件もない場合は「請求履歴はありません。」と表示されます

**領収書の表示**:
- 各請求の行にある「領収書を表示」のリンク（またはボタン）をクリックすると、領収書のページが別タブで開きます
- 領収書は決済代行の画面で表示されるため、印刷や保存はその画面から行えます

**請求期間について**:
- 請求期間は、ダッシュボードの月次統計（第3章）で表示している「請求期間」と同じです。プラン・請求ページで確認できる請求履歴の区切りと一致しています。`
      }
    ]
  },
  {
...
- **AI**: AIが生成した回答（緑色のバッジ）
- **スタッフ**: 管理者が会話詳細から送信した返信（アンバー色のバッジ）
...
- 匿名性を重視した設計ですが、ゲストが同意した場合のみ連絡先メールを受け付ける運用が可能です（施設設定・運用方針に従って実施）
- エスカレーション対応時は、会話内容、設置場所、時刻などの情報を活用して対応してください`
      },
      {
        id: 'conversation-reply',
        title: '9.4 エスカレーション対応について',
        content: `**手動返信機能について**:
- 会話詳細画面の「ゲストに返信」欄から、管理者が返信を投稿できます
- 返信は会話履歴に「スタッフ」として表示されます
- 未解決エスカレーションがある会話のみ返信を送信できます

**エスカレーション対応の実務的な流れ**:
1. **質問の確認**:
   - ダッシュボードの「未解決のエスカレーション」または「スタッフ不在時間帯対応キュー」で質問を確認します
   - 会話詳細画面で質問内容、設置場所、時刻、受付番号、連絡可能状態を確認します

2. **対応方法の決定**:
   - 質問内容に基づいて対応方法を決定します
   - 設置場所情報（例: 「客室」）から、対応が必要なエリアを推測できます
   - 時刻情報から、対応の緊急度を判断できます
   - 連絡可能状態が「連絡先あり」の場合は、施設運用ルールに従って連絡方法を選択します

3. **ゲストへの返信**:
   - 会話詳細画面の「ゲストに返信」欄に返信文を入力し、送信します
   - 投稿後、会話履歴に反映されたことを確認します
   - 必要に応じて、館内案内（掲示・口頭案内）と併用します
...
**エスカレーション対応時の注意点**:
- 受付番号はゲスト画面・管理画面・通知で同一です。必ず同じ番号で突合してください
- 連絡可能状態が「連絡先なし」の場合は、会話返信と館内案内を優先してください
...
- 緊急の場合は、館内放送や掲示板で案内することを推奨します`
  ...
  {
    id: 'guest',
    title: '第10章 ゲスト側の使い方（管理者向け説明）',
    subsections: [
      {
        id: 'guest-flow',
        title: '10.1 ゲストの利用フロー',
        content: `**ゲストの利用フロー**:
...
**利用可能な言語**:
- プランに応じて複数言語が利用可能になります（日本語、英語、繁体中国語、簡体中国語、フランス語、スペイン語、韓国語など。ドイツ語・ベトナム語は選択肢からは削除しています）
...
- 非表示にした後も、ブラウザのメニューから手動でホーム画面に追加できます`
      ...
      {
        id: 'guest-coupon-footer',
        title: '10.6 固定フッターとクーポン取得',
        content: `**固定フッターの表示条件**:
- 施設でクーポンが**有効**かつ**割引率を設定している**場合のみ、ゲストの言語選択・ウェルカム・チャット画面の**画面下部に固定フッター**が表示されます
- クーポンが無効、または割引率を未設定の施設では、クーポン用のフッターは表示されません

**フッターの表示内容**:
- 「オトクなクーポン」などのボタンが表示されます
- 表示文言は、ゲストが選択した言語（日本語・英語など）に応じて切り替わります

**クーポン取得の流れ**:
1. ゲストが固定フッターのボタンをタップします
2. エントリー画面（モーダルまたは専用画面）が開きます
3. お名前（任意）とメールアドレスを入力して送信します
4. 登録したメールアドレスに、施設名・割引率・利用条件・有効期限を記載したクーポン送付メールが届きます
5. 施設で公式サイトURLを設定している場合は、送付メールにそのURLが含まれ、ゲストが次回予約時に参照できます

**管理者向けのポイント**:
- クーポン設定は「第6章 施設設定」の「6.5 クーポン設定と公式サイトURL」で行います
- 取得者一覧は左メニュー「リード（クーポン取得）」で確認でき、CSVダウンロードも可能です（6.6参照）`
      }
    ]
  },
  {
    id: 'troubleshooting',
    title: '第11章 トラブルシューティング',
    subsections: [
      {
        id: 'trouble-faq',
        title: '11.1 よくある質問（FAQ）',
        content: `**ログインできない場合**:
...
- **対処法**:
  1. QRコードが正しく印刷されているか確認してください（第7章参照）
...
- 問題が発生する前に行った操作を詳しくお知らせください`
  ...
  {
    id: 'best-practices',
    title: '第12章 運用のベストプラクティス',
    subsections: [
      {
        id: 'practice-checklist',
        title: '12.1 初期設定チェックリスト',
        content: `**初期設定の流れ**:
1. **施設設定の確認**（第6章参照）
...
2. **スタッフ不在時間帯対応キューの確認**（第5章参照）
   ...
   - 必要に応じて、会話詳細からスタッフ返信を送信

3. **エスカレーション対応**
   - 未解決のエスカレーションを確認
   - 会話詳細画面で質問内容を確認
   - 受付番号を突合し、連絡可能状態（連絡先あり/なし）を確認して対応方針を決定
...
1. **月次統計の確認**（第3章参照）
   ...
   - プランの利用上限に近づいていないか確認
   - プラン・請求ページで現在のプランと請求履歴を確認する（第7章参照）
...
**請求期間**:
- 課金の区切りとなる期間。ダッシュボードの月次統計やプラン・請求ページの請求履歴と同じ区切りで表示されます。

**プラン**:
- 料金プランのこと。Free、Mini、Small、Standard、Premiumの5種類があります
- プランによって、FAQ登録数上限、対応言語数、質問数上限が異なります
- プランの確認・変更・解約は「プラン・請求」ページ（第7章参照）で行います`
      },
      {
        id: 'appendix-translation',
        title: '13.2 翻訳手引書（宿泊施設管理者向けFAQ翻訳ガイド）',
        content: `**翻訳手引書とは**:
...
3. **チャット画面** (\`/f/:facilityId/chat\`)
   ...
   - メッセージ入力、フィードバック、エスカレーション機能`
      },
      {
        ...
        title: '13.4 FAQ初期テンプレート例',
        content: `**FAQ初期テンプレートとは**:
- 新規施設登録時に自動的に登録されるFAQテンプレートです
- 30件のFAQテンプレートが初期自動登録されます
- Freeは日本語のみ、Miniは日・英、Smallは日・英・繁体中国語、Standardは日・英・繁体中国語・仏、Premiumは日本語・英語・繁体中国語・簡体中国語・フランス語・韓国語・スペイン語の7言語から選択可能です。ドイツ語・ベトナム語はゲスト画面の選択肢からは削除しています
...
**2026年2月**:
- 第13章付録に「13.2 翻訳手引書（宿泊施設管理者向けFAQ翻訳ガイド）」を追加
- 既存の12.2〜12.4を12.3〜12.5に繰り下げ

**2026年4月**:
- 第9章を現行仕様に更新（会話詳細でのスタッフ返信、受付番号突合、連絡可能状態の確認手順を反映）
- 第12章の日次運用を更新（未解決エスカレーション対応時の実務フローを明確化）
- C-3（同意ベース連絡先収集）の段階導入に合わせた運用注意を追記
...
**更新履歴の確認方法**:
- 本マニュアルの「13.5 更新履歴」セクションで確認できます
