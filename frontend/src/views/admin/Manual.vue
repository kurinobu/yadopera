...
**テンプレートの入手**:
- FAQ管理画面の「CSV一括登録」モーダル内にある「テンプレートダウンロード」リンクから、4言語（日本語・英語・フランス語・繁体中国語）の推奨テンプレートをダウンロードできます。マニュアルのこの節からも [CSVテンプレートをダウンロード](/faq-csv-template/FAQ_CSV_template_4lang.csv) できます。Premiumプランでは簡体中国語・スペイン語・韓国語も利用可能です。詳細はヘルプの「対応言語は何語ですか？」をご参照ください

**テンプレート形式・Excelで編集したい場合**:
- 配布のデフォルトは**CSV**です。CSVを扱ったことのない方は、Excelで編集することもできます。手順: テンプレートCSVをExcelで開く → 編集する →「ファイル」→「名前を付けて保存」→ ファイルの種類で「**CSV UTF-8（カンマ区切り）（.csv）**」を選んで保存してください
...
**エラー時**:
- タイムアウト（約60秒超過）、重複メッセージ（日本語で表示）、必須カラム不足などが表示されることがあります。表示されたメッセージに従ってCSVを修正し、再度アップロードしてください`
      }
    ]
  },
  {
